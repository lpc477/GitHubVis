...
const ProfileDetails = (props) => {
    return (
      <div>
        ...
        <div>
          {props.infoclean.location ? <div><p1>Location:</p1><p1>{props.infoclean.location}</p1></div> : null }
        </div>
        <div>
          {props.infoclean.company ? <div><p1>Company:</p1><p1>{props.infoclean.company}</p1></div> : null }
        </div>
        <div>
          {props.infoclean.public_repos ? <div><p1>Public Repos:</p1><p1>{props.infoclean.public_repos}</p1></div> : null }
        </div>
        <div>
          {props.infoclean.followers ? <div><p1>Followers:</p1><p1>{props.infoclean.followers}</p1></div> : null }
        </div>
        <div>
          {props.infoclean.following ? <div><p1>Following:</p1><p1>{props.infoclean.following}</p1></div> : null }
        </div>
        <div>
          {props.infoclean.html_url ? <div><p1><a rel="noopener noreferrer"
